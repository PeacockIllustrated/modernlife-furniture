...
- Type: two faces, the pairing the mark is built from. Montserrat states, loaded variable, with weight answering to size (280 at the largest display down to 500 for small heads) so the scale keeps one apparent colour. Anaktoria is the voice, not decoration: the manifesto, collector words, the hedge in an attribution, the aside under a label, the strapline. It has one weight and no italic, so never embolden or slant it. The highlighted word (the "Love is *personal*" pattern) stays in Montserrat at 500 and eases to 600 on hover through font-variation-settings, which is why the variable file matters. Sentence case everywhere, including nav and buttons.
